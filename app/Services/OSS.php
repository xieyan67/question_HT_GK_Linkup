...
class OSS
{
    ...
    public function __construct($isInternal = false)
    {
        ...
        $this->AccessKeyId = appConfig('oss', 'ACCESS_KEY_ID');
        $this->AccessKeySecret = appConfig('oss', 'ACCESS_KEY_SECRET');
     ...
     * @return boolean 上传是否成功
     */
    public static function publicUploadContent(
        $bucketName,
        $ossKey,
        $content,
        $options = []
     ...
     * @return boolean 上传是否成功
     */
    public static function privateUploadContent(
        $bucketName,
        $ossKey,
        $content,
        $options = []
    ...
    public static function getPrivateObjectURLWithExpireTime(
        $bucketName,
        $ossKey,
     ...
     * @param $directory
     * @return string|false if upload fail or file is not exist
     */
    public static function uploadFile(
        ...
        if (isset($myBucket)) {
            ...
            $bucketName = appConfig('oss', 'BUCKET');
        }
        $checkNum = md5(implode('', file($uploadFile->getPathname())));
        $extension = $uploadFile->extension();
        if ($uploadFile->isValid()) {
            $path = storage_path('app/public/').($uploadFile->storeAs(
                    $directory,
                    $checkNum,
                    ['disk' => 'public']
                ));
            $profileKey = $fileNamePrefix.$checkNum.'.'.$extension;
            if (self::publicUpload($bucketName, $profileKey, $path)) {
                $fileUrl = self::getPublicObjectURL($bucketName, $profileKey);
                // delete upload file
                unlink($path);
